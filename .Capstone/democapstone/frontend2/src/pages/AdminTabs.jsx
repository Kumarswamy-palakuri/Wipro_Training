...
export default function AdminTabs() {
  ...
  const [pending, setPending] = useState([]); const [assignRole, setAssignRole] = useState({});
  const loadPending = async () => { const { data } = await api.get("/approvals/pending"); setPending(data); setAssignRole(Object.fromEntries(data.map(p => [p.id, p.requestedRole || "Staff"]))); };
  ...
  const [users, setUsers] = useState([]); const [userRoles, setUserRoles] = useState({});
  const loadUsers = async () => { const { data } = await api.get("/admin/users"); setUsers(data); setUserRoles(Object.fromEntries(data.map(u => [u.id, u.role]))); };
  const saveUserRole = async (id) => { await api.put(`/admin/users/${id}/role`, { role: userRoles[id] }); };

  const [logs, setLogs] = useState([]); const [days, setDays] = useState(30);
  const loadLogs = async () => { const { data } = await api.get(`/inventory/movements?days=${days}`); setLogs(data); };

  const [inventory, setInventory] = useState([]); const loadInventory = async () => { const { data } = await api.get("/products"); setInventory(data); };
  const exportCSV = () => { if (inventory.length) downloadCSV("inventory.csv", inventory); };

  const currency = useMemo(() => new Intl.NumberFormat(undefined, { style: "currency", currency: "USD" }), []);

  useEffect(() => { loadPending(); loadUsers(); loadLogs(); loadInventory(); }, []);
    ...
    <div className="container" style={{ paddingTop: 24 }}>
      ...
      <TabPanel when="inventory">
        <Panel title="Inventory Export" right={<Button variant="primary" onClick={exportCSV}>Download CSV</Button>}>
          <div style={{ overflowX: "auto" }}>
            <table className="table">
              <thead><tr><th>ID</th><th>Name</th><th>Qty</th><th>Price</th><th>Total</th></tr></thead>
              <tbody>
                {inventory.length === 0 ? <tr><td colSpan="5" style={{ textAlign: "center" }}>No products</td></tr> :
                  inventory.map(p => <tr key={p.id}><td>{p.id}</td><td>{p.name}</td><td>{p.quantity}</td><td>{currency.format(p.price)}</td><td>{currency.format(p.price * p.quantity)}</td></tr>)}
              </tbody>
            </table>
          </div>
        </Panel>
