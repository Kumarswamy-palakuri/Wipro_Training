...
export default function ApprovalsPage() {
  ...
  const approve = async (id) => { try { await api.post(`/approvals/${id}/approve`, { role: assignRole[id] }); await load(); } catch { setError("Approve failed"); } };
  const reject = async (id) => { try { await api.post(`/approvals/${id}/reject`); await load(); } catch { setError("Reject failed"); } };
