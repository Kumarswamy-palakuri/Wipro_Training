...
export default function RegisterPage() {
  ...
  const submit = async () => {
    setError(null);
    try { await api.post("/registration", form); setDone(true); }
    catch (e) { setError(e?.response?.data || "Failed to submit"); }
  };
    ...
    <div className="container" style={{ maxWidth: 480, paddingTop: 60 }}>
      <Card>
        ...
        {done ? (
          ...
          <>
            <Field label="Username"><TextInput value={form.username} onChange={(e) => setForm({ ...form, username: e.target.value })} /></Field>
